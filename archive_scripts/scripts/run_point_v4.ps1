# Point v4 training entry.
# Usage example:
#   .\scripts\run_point_v4.ps1 -Experiment exp2
param(
...
Write-Host "[point_v4][$Experiment] output dir: $resolvedOutputDir"
Write-Host "[point_v4][$Experiment] config: $ConfigPath"
if (-not [string]::IsNullOrWhiteSpace($resolvedResumeCheckpoint)) {
    Write-Host "[point_v4][$Experiment] resume checkpoint: $resolvedResumeCheckpoint"
}
if ($resolvedTargetEpochs -gt 0) {
    Write-Host "[point_v4][$Experiment] target epochs: $resolvedTargetEpochs"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "[point_v4][$Experiment] training failed with exit code $LASTEXITCODE"
}

Save-LastRunDir $resolvedOutputDir

if (-not $SkipReport) {
    Write-Host "[point_v4][$Experiment] training finished, generating report..."
    & ".\\scripts\\make_point_v4_report.ps1" `
