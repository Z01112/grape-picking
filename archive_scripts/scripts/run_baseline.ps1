...
$configPath = Join-Path $repoRoot "configs\rtv4\rtv4_hgnetv2_s_grape_picking_baseline.yml"
...
function Resolve-OutputDir {
    ...
    if (-not [string]::IsNullOrWhiteSpace($PinnedOutputDir)) {
        return [System.IO.Path]::GetFullPath((Join-Path $repoRoot $PinnedOutputDir))
    }

    $dateTag = Get-Date -Format "yyyyMMdd"
    $baseDir = Join-Path "outputs" ("baseline_" + $dateTag)
    $candidate = $baseDir
    $index = 1
    while (Test-Path -LiteralPath $candidate) {
        $candidate = "{0}_{1}" -f $baseDir, $index
        $index += 1
    }
...
Write-Host "[baseline] output dir: $resolvedOutputDir"
if (-not [string]::IsNullOrWhiteSpace($resolvedResumeCheckpoint)) {
    Write-Host "[baseline] resume checkpoint: $resolvedResumeCheckpoint"
}
if ($resolvedTargetEpochs -gt 0) {
    Write-Host "[baseline] target epochs: $resolvedTargetEpochs"
}
...
if (-not $SkipReport) {
    Write-Host "[baseline] training finished, generating report..."
