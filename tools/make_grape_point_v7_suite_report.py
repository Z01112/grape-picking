...
DEFAULT_EXP_SUMMARIES = {
    "exp1": REPO_ROOT / "outputs" / "grape_point_v7_exp1_query_box_top_center" / "report" / "summary.json",
    "exp2": REPO_ROOT / "outputs" / "grape_point_v7_exp2_query_box_top_center_toproi" / "report" / "summary.json",
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Aggregate point_v7 experiment summaries.")
    parser.add_argument("--suite-dir", type=Path, default=REPO_ROOT / "outputs" / "grape_point_v7_suite" / "report")
...
def build_report_md(report_path: Path, baseline_metrics: dict | None, exp_results: dict[str, dict], best_exp: str | None) -> None:
    lines = [
        ...
        "",
        "## 总览",
        "- point_v7 延续 point_v6 的单阶段 per-grape has_picking + point offset 主线，不回退到 picking bbox。",
        "- 本轮只推进两项与当前主矛盾直接相关的改动：top-center 坐标表达，以及 query_box 与顶部局部视觉 cue 的联合建模。",
        "- 目标不是做工业级零误差系统，而是在论文设定下把 grape 检测、point 定位和弱关联链条一起做稳。",
        "",
    ]
    if baseline_metrics is not None:
        lines.append(
    ...
    if best_exp and exp_results.get(best_exp) is not None:
        ...
        lines.extend(
            [
                "",
                "## 当前自动推荐实验",
                f"- 推荐 `{best_exp}` 作为 point_v7 当前主线。",
    ...
    lines.extend(["", "## 各实验摘要"])
    name_note = {
        "exp1": "query_box + top_center",
        "exp2": "query_box_top + top_center",
    }
    for name in ("exp1", "exp2"):
        metrics = exp_results.get(name)
        if metrics is None:
            ...
            continue
        lines.append(
            f"- `{name}` ({name_note.get(name, '')}): grape AP={metrics['grape_AP']:.4f}, has_picking F1={metrics['has_picking_F1']:.4f}, pair_count={metrics['pair_count']}, mean L2={metrics['mean_L2']:.2f}px, median L2={metrics['median_L2']:.2f}px, p90 L2={metrics['p90_L2']:.2f}px, |dx|={metrics['mean_abs_dx']:.2f}px, |dy|={metrics['mean_abs_dy']:.2f}px, small/medium/large L2={metrics['small_L2']:.2f}/{metrics['medium_L2']:.2f}/{metrics['large_L2']:.2f}px, cross-instance mismatch={metrics['cross_instance_mismatch_count']}。"
        )
    lines.extend(
        [
            "",
            "## 关注点",
            "- 如果 AP、F1、pair_count 一起提升，但 mean L2 不降，说明关联链条改善了而 point 头本身还没跟上。",
            "- 如果 |dy| 明显下降，说明 top-center 这类上部锚点表达确实对当前采摘点任务更合适。",
            "- 如果 cross-instance mismatch 仍然很低，就不该再把论文主矛盾写成严格实例一一绑定失败。",
        ]
    )
