...
DEFAULT_CONFIG = REPO_ROOT / "configs" / "rtv4" / "rtv4_hgnetv2_s_grape_point_v7_exp2_tight_toproi.yml"
...
def main() -> None:
    ...
    if "--report-mode" not in sys.argv:
        sys.argv.extend(["--report-mode", "point_v7_exp2_tight_toproi_full"])
    if "--reference-label" not in sys.argv:
        sys.argv.extend(["--reference-label", "baseline_replay"])
    if "--report-title" not in sys.argv:
        sys.argv.extend(["--report-title", "point_v7_exp2 tight top-local ROI 中文结论"])
