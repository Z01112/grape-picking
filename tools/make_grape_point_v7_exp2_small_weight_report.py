...
DEFAULT_CONFIG = REPO_ROOT / "configs" / "rtv4" / "rtv4_hgnetv2_s_grape_point_v7_exp2_small_weight.yml"
...
def main() -> None:
    ...
    if "--report-mode" not in sys.argv:
        sys.argv.extend(["--report-mode", "point_v7_exp2_small_weight_full"])
    if "--reference-label" not in sys.argv:
        sys.argv.extend(["--reference-label", "baseline_replay"])
    if "--report-title" not in sys.argv:
        sys.argv.extend(["--report-title", "point_v7_exp2_small_weight 中文结论"])
