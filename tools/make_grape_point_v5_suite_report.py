...
DEFAULT_BASELINE_REPLAY_SUMMARY = REPO_ROOT / "outputs" / "grape_point_v5_baseline_replay" / "report" / "summary.json"
DEFAULT_POINT_V2_SUMMARY = REPO_ROOT / "outputs" / "grape_point_v2_main" / "report" / "summary.json"
DEFAULT_EXP_SUMMARIES = {
    "exp1": REPO_ROOT / "outputs" / "grape_point_v5_exp1_dense" / "report" / "summary.json",
    "exp2": REPO_ROOT / "outputs" / "grape_point_v5_exp2_geo" / "report" / "summary.json",
    "exp3": REPO_ROOT / "outputs" / "grape_point_v5_exp3_dense_geo" / "report" / "summary.json",
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Aggregate point_v5 experiment summaries.")
    parser.add_argument("--suite-dir", type=Path, default=REPO_ROOT / "outputs" / "grape_point_v5_suite" / "report")
...
def choose_best_experiment(results: dict[str, dict], baseline_metrics: dict | None) -> str | None:
    if baseline_metrics is None:
        valid = [(m["mean_L2"], n) for n, m in results.items() if m is not None and np.isfinite(m["mean_L2"])]
        if not valid:
            return None
        valid.sort()
        return valid[0][1]
    ...
    for name, metrics in results.items():
        if metrics is None or not np.isfinite(metrics["mean_L2"]):
            continue
        ...
        min_pair = int(round(0.90 * baseline_metrics["pair_count"]))
        if metrics["pair_count"] < min_pair:
            score += float(min_pair - metrics["pair_count"]) * 0.15
...
def build_report_md(report_path: Path, baseline_metrics: dict | None, exp_results: dict[str, dict], best_exp: str | None) -> None:
    lines = [
        ...
        "",
        "## 总览",
        "- point_v5 直接从 point_v2 重新起线，不继承 point_v3 / point_v4 的 ROI feature 注入路线。",
        "- 这轮只验证两项改动：training-only dense supervision 和 geometric consistency。",
        "",
    ]
    if baseline_metrics is not None:
        lines.append(
            f"- baseline_replay：grape AP={baseline_metrics['grape_AP']:.4f}, has_picking F1={baseline_metrics['has_picking_F1']:.4f}, pair_count={baseline_metrics['pair_count']}, mean L2={baseline_metrics['mean_L2']:.2f}px, median L2={baseline_metrics['median_L2']:.2f}px, p90 L2={baseline_metrics['p90_L2']:.2f}px。"
        )
    if best_exp and exp_results.get(best_exp) is not None:
        ...
        lines.extend(
            [
                "",
                "## 当前自动推荐实验",
                f"- 推荐 `{best_exp}` 作为 point_v5 当前最值得细看的实验。",
                f"- test 指标：grape AP={best['grape_AP']:.4f}, has_picking F1={best['has_picking_F1']:.4f}, pair_count={best['pair_count']}, mean L2={best['mean_L2']:.2f}px, median L2={best['median_L2']:.2f}px, p90 L2={best['p90_L2']:.2f}px。",
            ]
        )
        if baseline_metrics is not None:
            lines.append(
                f"- 相比 baseline_replay：Δgrape AP={best['grape_AP'] - baseline_metrics['grape_AP']:+.4f}, Δhas_picking F1={best['has_picking_F1'] - baseline_metrics['has_picking_F1']:+.4f}, Δmean L2={best['mean_L2'] - baseline_metrics['mean_L2']:+.2f}px。"
            )
    lines.extend(["", "## 各实验摘要"])
    for name in ("exp1", "exp2", "exp3"):
        metrics = exp_results.get(name)
