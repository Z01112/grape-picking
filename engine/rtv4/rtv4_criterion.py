...
class RTv4Criterion(nn.Module):
    ...
    def _collect_dense_positive_examples(self, outputs, targets, indices):
        ...
        if pred_boxes is None or pred_has is None or pred_offsets is None:
            self._dense_positive_cache = None
            return None
        if self.dense_positive_topk <= 0 or self.dense_positive_iou_thresh <= 0.0:
            self._dense_positive_cache = None
            return None
        ...
        for batch_id, target in enumerate(targets):
            ...
            for gt_id in range(gt_boxes.shape[0]):
                candidate_pred = torch.nonzero(
                    (best_gt == gt_id) & (best_ious >= self.dense_positive_iou_thresh),
                    as_tuple=False,
                ).flatten()
                ...
                for pred_id in sorted_pred.tolist():
                    ...
                    kept += 1
                    if kept >= self.dense_positive_topk:
                        break

        if not batch_indices:
            self._dense_positive_cache = {
