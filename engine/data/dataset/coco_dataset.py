...
class ConvertCocoPolysToMask(object):
    ...
    def __call__(self, image: Image.Image, target, **kwargs):
        ...
        has_picking = None
        if anno and any("has_picking" in obj for obj in anno):
            has_picking = torch.as_tensor(
                [float(obj.get("has_picking", 0.0)) for obj in anno],
                dtype=torch.float32,
            )
        ...
        stem_bbox = None
        if anno and any("stem_bbox" in obj for obj in anno):
            stem_bbox = torch.as_tensor(
                [
                    obj.get("stem_bbox", [0.0, 0.0, 0.0, 0.0])[:4]
                    if obj.get("stem_bbox") is not None
        ...
        if anno and any("picking_point" in obj for obj in anno):
            picking_points = torch.as_tensor(
                [
                    obj.get("picking_point", [0.0, 0.0])[:2]
                    if obj.get("picking_point") is not None
                    else [0.0, 0.0]
                    for obj in anno
                ],
                dtype=torch.float32,
        ...
        if anno and any("picking_offset" in obj for obj in anno):
            picking_offsets = torch.as_tensor(
                [
                    obj.get("picking_offset", [0.0, 0.0])[:2]
                    if obj.get("picking_offset") is not None
                    else [0.0, 0.0]
                    for obj in anno
                ],
                dtype=torch.float32,
        ...
        if stem_bbox is not None:
            target["stem_bbox"] = stem_bbox
        if picking_points is not None:
            target["picking_points"] = picking_points
        if picking_offsets is not None:
            target["picking_offsets"] = picking_offsets
