...
Write-Host "[point_v6][$Experiment] output dir: $resolvedOutputDir"
Write-Host "[point_v6][$Experiment] config: $ConfigPath"
if (-not [string]::IsNullOrWhiteSpace($resolvedResumeCheckpoint)) {
    Write-Host "[point_v6][$Experiment] resume checkpoint: $resolvedResumeCheckpoint"
}
if ($resolvedTargetEpochs -gt 0) {
    Write-Host "[point_v6][$Experiment] target epochs: $resolvedTargetEpochs"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "[point_v6][$Experiment] training failed with exit code $LASTEXITCODE"
}

Save-LastRunDir $resolvedOutputDir

if (-not $SkipReport) {
    Write-Host "[point_v6][$Experiment] training finished, generating report..."
    & ".\\scripts\\make_point_v6_report.ps1" `
