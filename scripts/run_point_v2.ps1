...
$PinnedOutputDir = "outputs\grape_point_v2_main"
$LastRunFile = Join-Path $repoRoot "scripts\.last_point_v2_run.txt"
$configPath = Join-Path $repoRoot "configs\rtv4\rtv4_hgnetv2_s_grape_point_v2.yml"
...
Write-Host "[point_v2] output dir: $resolvedOutputDir"
if (-not [string]::IsNullOrWhiteSpace($resolvedResumeCheckpoint)) {
    Write-Host "[point_v2] resume checkpoint: $resolvedResumeCheckpoint"
}
if ($resolvedTargetEpochs -gt 0) {
    Write-Host "[point_v2] target epochs: $resolvedTargetEpochs"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "[point_v2] training failed with exit code $LASTEXITCODE"
}

Save-LastRunDir $resolvedOutputDir

if (-not $SkipReport) {
    Write-Host "[point_v2] training finished, generating report..."
    & ".\\scripts\\make_point_v2_report.ps1" `
